      ...
      <mat-card>
        <mat-card-header>
          ...
          <div class="header-actions">
            <button mat-raised-button color="primary" routerLink="/goods-receipts/new">
              <mat-icon>add</mat-icon> Create GR
            </button>
          </div>
        </mat-card-header>
        <mat-card-content>
          <div class="filters" [formGroup]="filterForm">
            <mat-form-field appearance="outline">
              ...
              <input matInput formControlName="search" placeholder="GR#, PO#, Supplier...">
          ...
          @if (loading()) {
            ...
            <table mat-table [dataSource]="filteredGRs()" class="full-width">
              <ng-container matColumnDef="grNumber">
                <th mat-header-cell *matHeaderCellDef>GR #</th>
                <td mat-cell *matCellDef="let gr">{{ gr.grNumber }}</td>
              </ng-container>
              ...
              <ng-container matColumnDef="receivedDate">
                ...
                <td mat-cell *matCellDef="let gr">{{ gr.receivedDate | date:'mediumDate' }}</td>
              </ng-container>
              ...
              <ng-container matColumnDef="actions">
                ...
                <td mat-cell *matCellDef="let gr">
                  <button mat-icon-button color="primary" [routerLink]="['/goods-receipts', gr.id]" matTooltip="View">
...
export class GrListComponent implements OnInit {
  ...
  grs = signal<GoodsReceipt[]>([]);
  displayedColumns = ['grNumber', 'poNumber', 'supplier', 'receivedDate', 'status', 'actions'];

  filterForm = this.fb.group({ search: [''], status: [''] });

  filteredGRs = computed(() => {
    const { search, status } = this.filterForm.value;
    return this.grs().filter(gr => {
      const matchSearch = !search || [gr.grNumber, gr.poNumber, gr.supplier]
        .some(v => v.toLowerCase().includes(search.toLowerCase()));
      const matchStatus = !status || gr.status === status;
  ...
  load() {
    ...
    this.api.get<GoodsReceipt[]>('/goods-receipts').subscribe({
      next: data => { this.grs.set(data); this.loading.set(false); },
      error: () => { this.notification.error('Failed to load goods receipts'); this.loading.set(false); }
    });
  }
}
