...
public class InvoicesController(IMediator mediator) : ControllerBase
{
    ...
    [HttpPost("{id}/approve"), Authorize(Roles = "Admin,Finance,Approver")] public async Task<IActionResult> Approve(string id, CancellationToken ct) => Ok(await mediator.Send(new ApproveInvoiceCommand(id), ct));
    [HttpPost("{id}/reject"), Authorize(Roles = "Admin,Finance,Approver")] public async Task<IActionResult> Reject(string id, [FromBody] RejectInvoiceBody body, CancellationToken ct) => Ok(await mediator.Send(new RejectInvoiceCommand(id, body.Reason), ct));
    [HttpPost("{id}/schedule-payment"), Authorize(Roles = "Admin,Finance,Approver")] public async Task<IActionResult> SchedulePayment(string id, [FromBody] SchedulePaymentBody body, CancellationToken ct) => Ok(await mediator.Send(new SchedulePaymentCommand(id, body.PaymentDate), ct));
    [HttpPost("{id}/mark-paid"), Authorize(Roles = "Admin,Finance,Approver")] public async Task<IActionResult> MarkPaid(string id, [FromBody] MarkPaidBody body, CancellationToken ct) => Ok(await mediator.Send(new MarkInvoicePaidCommand(id, body.PaymentReference), ct));
